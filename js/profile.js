...
async function checkAuth() {
    ...
    if (window.updateProfileDropdown) {
        ...
        window.updateProfileDropdown(true, name, avatarUrl);
    }
    loadProfile();
}
...
async function loadProfile() {
    try {
        ...
        if (profile) {
            ...
            if (sidebarName) {
                sidebarName.innerHTML = window.safeHTML(profile.name || user.email?.split('@')[0] || 'Account');
            }

            // Update Avatars
            // Order of preference: 1. DB avatar_url, 2. Google Metadata icon
            let displayAvatarUrl = profile.avatar_url;

            if (!displayAvatarUrl) {
                // Try Google metadata picture
                const googlePic = user.user_metadata?.avatar_url || user.user_metadata?.picture;
                if (googlePic) {
                    displayAvatarUrl = googlePic;
                    // Sync with DB silently if profile exists
                    if (profile) {
                        window.supabaseClient
                            .from('profiles')
                            .update({ avatar_url: googlePic })
                            .eq('id', user.id)
                            .then(() => console.log('Google profile picture synced to DB'));
                    }
                }
            }
            ...
            if (displayAvatarUrl) {
                ...
                // Sidebar: Image only
                if (sidebarAvatar) {
                    sidebarAvatar.innerHTML = `<img src="${displayAvatarUrl}" class="w-full h-full object-cover">`;
                }

                // Header: Image + Overlay
                if (headerAvatar) {
                    headerAvatar.innerHTML = `
                        <img src="${displayAvatarUrl}" class="w-full h-full object-cover">
...
async function updateProfile(event) {
    ...
    try {
        ...
        if (fileInput && fileInput.files.length > 0) {
            ...
            const filePath = `${user.id}/${fileName}`; // Folder-based for RLS
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 ${addr.is_default ? 'border-gold bg-orange-50' : ''}">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                ...
                                <h4 class="font-bold text-gray-800 mb-1">${window.safeHTML(addr.name)}</h4>
                                <p class="text-sm text-gray-600">${window.safeHTML(addr.address)}</p>
                                <p class="text-sm text-gray-600">${window.safeHTML(addr.city)}, ${window.safeHTML(addr.state)} - ${window.safeHTML(addr.pincode)}</p>
                                <p class="text-sm text-gray-600 mt-1">Phone: ${window.safeHTML(addr.phone)}</p>
                            </div>
    ...
    try {
        ...
        const addressHtml = `
            <div class="text-sm text-gray-600">
                <div class="font-bold text-gray-800">${window.safeHTML(address.name) || 'N/A'}</div>
                <div>${window.safeHTML(address.address) || ''}</div>
                <div>${window.safeHTML(address.city) || ''}, ${window.safeHTML(address.state) || ''} - ${window.safeHTML(address.pincode) || ''}</div>
                <div>Phone: ${window.safeHTML(address.phone) || ''}</div>
            </div>
        `;
